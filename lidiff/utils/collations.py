...
def point_set_to_sparse(p_full, p_part, n_full, n_part, resolution, filename, p_mean=None, p_std=None):
    ...
    # For Open3D 0.15.1 compatibility
    if hasattr(pcd_part, 'farthest_point_down_sample'):
        pcd_part = pcd_part.farthest_point_down_sample(n_part)
    else:
        # Use voxel_down_sample as alternative
        voxel_size = np.linalg.norm(p_part.max(axis=0) - p_part.min(axis=0)) / np.cbrt(n_part)
        pcd_part = pcd_part.voxel_down_sample(voxel_size=voxel_size)
        
        # If not enough points, repeat and sample
        if len(pcd_part.points) < n_part:
            p_part = np.array(pcd_part.points)
            concat_needed = np.ceil(n_part / len(p_part))
            p_part = np.repeat(p_part, concat_needed, axis=0)
            p_part = p_part[:n_part]
            p_part = torch.tensor(p_part)
        else:
            # Randomly sample to get exactly n_part points
            p_part = np.array(pcd_part.points)
            indices = np.random.choice(len(p_part), n_part, replace=False)
            p_part = p_part[indices]
            p_part = torch.tensor(p_part)
    
    if not isinstance(p_part, torch.Tensor):
