...
class DiffCompletion(LightningModule):
    ...
    def preprocess_scan(self, scan):
        ...
        pcd_scan = pcd_scan.farthest_point_down_sample(int(self.hparams['data']['num_points'] / 25 ))
...
@click.command()
@click.option('--diff', '-d', type=str, default='/home/fang/Downloads/LiDiff-main/lidiff/checkpoints/diff_net.ckpt', help='path to the scan sequence')
@click.option('--refine', '-r', type=str, default='/home/fang/Downloads/LiDiff-main/lidiff/checkpoints/refine_net.ckpt', help='path to the scan sequence')
@click.option('--denoising_steps', '-T', type=int, default=30, help='number of denoising steps (default: 50)')
...
def main(diff, refine, denoising_steps, cond_weight):
    ...
    path = '/media/fang/extra/dataset/process'

    os.makedirs(f'/media/fang/extra/results/{exp_dir}/refine', exist_ok=True)
    os.makedirs(f'/media/fang/extra/results/{exp_dir}/diff', exist_ok=True)
    ...
    # 收集所有子子文件夹中的PCD文件路径
    pcd_files = []
    for subdir in natsorted(os.listdir(path)):
        subdir_path = os.path.join(path, subdir)
        if not os.path.isdir(subdir_path):
            continue
        # 获取子子文件夹（假设每个子文件夹下只有一个子子文件夹）
        subsubdirs = os.listdir(subdir_path)
        if not subsubdirs:
            continue
        subsubdir = subsubdirs[0]
        subsubdir_path = os.path.join(subdir_path, subsubdir)
        if not os.path.isdir(subsubdir_path):
            continue
        # 遍历子子文件夹中的文件
        for filename in natsorted(os.listdir(subsubdir_path)):
            file_path = os.path.join(subsubdir_path, filename)
            if os.path.isfile(file_path):
                pcd_files.append(file_path)

    # 处理所有收集到的文件
    for pcd_file in tqdm.tqdm(pcd_files):
        # 生成保存路径
        relative_path = os.path.relpath(pcd_file, path)
        relative_dir = os.path.dirname(relative_path)
        filename = os.path.basename(pcd_file).split('.')[0]

        # 创建结果子目录
        refine_save_dir = os.path.join(f'/media/fang/extra/results/{exp_dir}/refine', relative_dir)
        diff_save_dir = os.path.join(f'/media/fang/extra/results/{exp_dir}/diff', relative_dir)
        # refine_save_dir = os.path.join(f'./results/{exp_dir}/refine', relative_dir)
        # diff_save_dir = os.path.join(f'./results/{exp_dir}/diff', relative_dir)
        os.makedirs(refine_save_dir, exist_ok=True)
        ...
        o3d.io.write_point_cloud(os.path.join(refine_save_dir, f'{filename}.ply'), pcd_refine)

        pcd_diff = o3d.geometry.PointCloud()
        pcd_diff.points = o3d.utility.Vector3dVector(diff_scan)
        pcd_diff.estimate_normals()
        o3d.io.write_point_cloud(os.path.join(diff_save_dir, f'{filename}.ply'), pcd_diff)
